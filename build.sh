#!/usr/bin/env bash
# Script de build pour Vercel – génère frontend/config.js avec l'URL du backend.
# Définir API_BASE_URL dans les variables d'environnement du projet Vercel.
set -e

if [ -z "${API_BASE_URL:-}" ]; then
    echo "⚠️  WARNING: API_BASE_URL n'est pas définie. Le frontend ne pourra pas contacter le backend."
    echo "   → Ajoute API_BASE_URL dans les variables d'environnement Vercel."
    API_URL=""
else
